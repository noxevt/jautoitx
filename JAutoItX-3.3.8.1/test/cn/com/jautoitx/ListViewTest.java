...
public class ListViewTest extends BaseTest {
	...
	public void deSelect() {
		...
		addFileToListView(new File("test/HashMyFiles.exe"), 1);
		addFileToListView(new File("test/iniDelete.tmp"), 2);
	...
	public void findItem() {
		...
		// add file to ListView
		addFileToListView(new File("test/你好.txt"), 2);
	...
	private void addFileToListView(File file, int expectedItemCount) {
		addFileToListView(file.getAbsolutePath(), expectedItemCount);
	}

	private void addFileToListView(String filePath, int expectedItemCount) {
		// add file to ListView
		Assert.assertTrue(Win.menuSelectItem("HashMyFiles", null, "&File",
				"Add &Files"));
		Assert.assertTrue(Win.active(HASH_MY_FILES_ADD_FILES_TITLE));
		sleep(500);
		Control.setText(HASH_MY_FILES_ADD_FILES_TITLE, "Edit1", filePath);
		Keyboard.send("{ENTER}");
		sleep(1000);
		assertEquals(expectedItemCount,
				ListView.getItemCount(HASH_MY_FILES_TITLE, "SysListView321"));
	}
