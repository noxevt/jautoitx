...
public class Process extends AutoItX {
	 ...
	 * 
	 * Note: AutoItX has bug to unset the RunAs details
	 * 
	 * @return Return false if the operating system does not support this
	 *         function, otherwise returns true--regardless of success.
	 * @see #runAsSet(String, String, String)
	 */
	public static boolean runAsSet() {
		// Note: AutoItX has bug to unset the RunAs details

		if (runAsDetailsSetted) {
			runAsDetailsSetted = false;
			return true;
		}
	 ...
	 * 
	 * This function allows subsequent Run and RunWait functions to run as a
	 * different user (e.g. Administrator). The function only works on the
	 * 2000/XP (or later) platforms. NT4 users should install and use the SU
	 * command from the NT Resource Kit.
	 * 
	 * The "Secondary Logon service" or "RunAs service" must not be disabled if
	 * you want this function to work.
	 * 
	 ...
	 *            The password to use.
	 * @return Return false if the operating system does not support this
	 *         function, Otherwise returns true--regardless of success. (If the
	 *         login information was invalid, subsequent Run/RunWait commands
	 *         will fail....)
	 */
	public static boolean runAsSet(final String user, final String domain,
			final String password) {
		return runAsSet(user, domain, password, (Integer) null);
	}
	 ...
	 */
	public static boolean runAsSet(final String user, final String domain,
			final String password, final Integer options) {
		// To unset the RunAs details, use the function with no parameters:
		// RunAsSet()
		if (StringUtils.isEmpty(user) && StringUtils.isEmpty(domain)
				&& StringUtils.isEmpty(password) && (options == null)) {
			return runAsSet();
		}
	...
	public static RunWaitResult runWait(final String fileName,
		...
		int exitCode = autoItXLib.AU3_RunWait(
				...
				stringToWString(workingDir), (flag == null) ? SW_SHOWNORMAL
						: flag);
		return new RunWaitResult(autoItXLib.AU3_error(), exitCode);
	}
	...
	/**
	 * The result for Process.runWait method.
	 * 
	 * @author zhengbo.wang
	 */
