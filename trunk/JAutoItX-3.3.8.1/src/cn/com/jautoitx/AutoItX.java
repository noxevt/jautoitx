...
public class AutoItX {
	...
	protected static interface AutoItXLibrary extends Library {
		 ...
		 * <tr>
		 * <td>Windows 98/Me</td>
		 * <td>User input is blocked but AutoIt is also unable to simulate
		 * input.</td>
		 * </tr>
		 * <tr>
		 ...
		 * </table>
		 * <br>
		 * Note that functions such as WinMove() will still work on Windows
		 * 98/Me when BlockInput is enabled.
		 * 
		 * @param flag
		 *            1 = Disable user input<br>
		 *            0 = Enable user input
		 */
		public void AU3_BlockInput(final int flag);

		/**
		 * Opens or closes the CD tray.
		 * 
		 * CDTray works as expected with virtual CD drives such as DAEMON Tools.
		 * CDTray does not work on non-local/mapped CD drives; CDTray must be
		 * run from the computer whose drive it affects. CDTray("X:", "close")
		 * tends to return 0 even on laptop-style cd trays that can only be
		 * closed manually.
		 * 
		 * @param drive
		 *            The drive letter of the CD tray to control, in the format
		 *            D:, E:, etc.
		 * @param status
		 *            Specifies if you want the CD tray to be open or closed:
		 *            "open" or "closed"
		 * @return Returns 1 if success, returns 0 if drive is locked via CD
		 *         burning software or if the drive letter is not a CD drive.
		 */
		public int AU3_CDTray(final WString drive, final WString status);
		 ...
		 * @param bufSize
		 */
		public void AU3_ControlGetHandle(final WString title,
				final WString text, final WString control,
		 ...
		 * @param control
		 *            The control to interact with.
		 * @return Returns the X coordinate of the control if success, sets
		 *         oAutoIt.error to 1 if failed.
		 */
		public int AU3_ControlGetPosX(final WString title, final WString text,
				final WString control);
		 ...
		 * @param control
		 *            The control to interact with.
		 * @return Returns the Y coordinate of the control if success, sets
		 *         oAutoIt.error to 1 if failed.
		 */
		public int AU3_ControlGetPosY(final WString title, final WString text,
				final WString control);
		 ...
		 * @param control
		 *            The control to interact with.
		 * @return Returns the height of the control if success, sets
		 *         oAutoIt.error to 1 if failed.
		 */
		public int AU3_ControlGetPosHeight(final WString title,
				final WString text, final WString control);
		 ...
		 * @param control
		 *            The control to interact with.
		 * @return Returns the width of the control if success, sets
		 *         oAutoIt.error to 1 if failed.
		 */
		public int AU3_ControlGetPosWidth(final WString title,
				final WString text, final WString control);
		 ...
		 * @param key
		 *            The key name in the in the .ini file. If no key name is
		 *            given the entire section is deleted.
		 * @return Return 1 if success, return 0 if section/key is not found or
		 *         if INI file is read-only.
		 */
		public long AU3_IniDelete(final WString fileName,
				final WString section, final WString key);
		 ...
		 * @param bufSize
		 */
		public void AU3_IniRead(final WString fileName, final WString section,
				final WString key, final WString defaultValue,
				final CharBuffer value, final int bufSize);
		 ...
		 * [SectionName]<br>
		 * Key=Value<br>
		 * If file does not exist, it is created. Keys and/or sections are added
		 * to the end and are not sorted in any way.
		 * 
		 * @param fileName
		 *            The filename of the .ini file.
		 * @param section
		 ...
		 */
		public int AU3_IniWrite(final WString fileName, final WString section,
				final WString key, final WString value);
		 ...
		 */
		public int AU3_PixelChecksum(final int left, final int top,
				final int right, final int bottom, final Integer step);
		 ...
		 * 
		 * @return Return 0 if the operating system does not support this
		 *         function, otherwise returns 1--regardless of success.
		 * @see #AU3_RunAsSet(String, String, String)
		 */
		public int AU3_RunAsSet();

		/**
		 * Initialise a set of user credentials to use during Run and RunWait
		 * operations. 2000/XP or later ONLY.
		 * 
		 * * This function allows subsequent Run and RunWait functions to run as
		 * a different user (e.g. Administrator). The function only works on the
		 * 2000/XP (or later) platforms. NT4 users should install and use the SU
		 * command from the NT Resource Kit.
		 * 
		 * The "Secondary Logon service" or "RunAs service" must not be disabled
		 * if you want this function to work.
		 * 
		 * To unset the RunAs details, use the function with no parameters:
		 * RunAsSet().
		 * 
		 * @param user
		 ...
		 *            The password to use.
		 * @param options
		 *            0 = do not load the user profile, 1 = (default) load the
		 *            user profile, 2 = use for net credentials only
		 * @return Return 0 if the operating system does not support this
		 *         function, otherwise returns 1--regardless of success. (If the
		 *         login information was invalid, subsequent Run/RunWait
		 *         commands will fail....)
		 */
		public int AU3_RunAsSet(final WString user, final WString domain,
				final WString password, final Integer options);
		 ...
		 * 
		 * @return Returns the X coordinate of the caret if success, sets
		 *         oAutoIt.error to 1 if failed.
		 */
		 ...
		 * coordinates regardless of caret position!
		 * 
		 * @return Returns the Y coordinate of the caret if success, sets
		 *         oAutoIt.error to 1 if failed.
		 */
		 ...
		 *         returns numeric 1 and sets oAutoIt.error to 1 if windows is
		 *         not found.
		 */
		public int AU3_WinGetClientSizeHeight(final WString title,
				final WString text);
		 ...
		 * @param text
		 *            The text of the window to read.
		 * @return Returns the width of the window's client area if success,
		 *         returns numeric 1 and sets oAutoIt.error to 1 if windows is
		 *         not found.
		 */
		public int AU3_WinGetClientSizeWidth(final WString title,
				final WString text);
		 ...
		 * @param bufSize
		 */
		public void AU3_WinGetHandle(final WString title, final WString text,
				final CharBuffer retText, int bufSize);
		 ...
		 * @param text
		 *            The text of the window to read.
		 * @return Returns the X coordinate of the window if success, return
		 *         numeric 1 and sets oAutoIt.error to 1 if windows is not
		 *         found.
		 */
		public int AU3_WinGetPosX(final WString title, final WString text);
		 ...
		 * @param text
		 *            The text of the window to read.
		 * @return Returns the Y coordinate of the window if success, return
		 *         numeric 1 and sets oAutoIt.error to 1 if windows is not
		 *         found.
		 */
		public int AU3_WinGetPosY(final WString title, final WString text);
		 ...
		 * @param text
		 *            The text of the window to read.
		 * @return Returns the height of the window if success, returns numeric
		 *         1 and sets oAutoIt.error to 1 if windows is not found.
		 */
		public int AU3_WinGetPosHeight(final WString title, final WString text);
		 ...
		 * @param text
		 *            The text of the window to read.
		 * @return Returns the width of the window if success, returns numeric 1
		 *         and sets oAutoIt.error to 1 if windows is not found.
		 */
		public int AU3_WinGetPosWidth(final WString title, final WString text);
		 ...
		 * @param bufSize
		 */
		public void AU3_WinGetProcess(final WString title, final WString text,
				final CharBuffer retText, int bufSize);
